<template>
  ...
  <el-dialog title="过滤规则配置" :visible.sync="dialogVisible" width="50%" append-to-body :before-close="handleClose">
...
export default {
  ...
  methods: {
    init(data) {
      this.pconditions = data.pconditions
      this.pconditionsAddtion = data.pconditionsAddtion
      this.chooseNode = data.chooseNode
    },
    confirm() {
      // 获取属性配置
      this.pconditions = this.$refs.base.getData()
      this.pconditionsAddtion = this.$refs.addtion.getData()
      let desc = ''
      ...
      this.$emit('ruleConfig', {
        ...
        pconditionsAddtion: this.pconditionsAddtion,
        chooseNode: this.chooseNode,
        desc: '条件:' + desc + ',变更节点==>' + this.chooseNode
      })
    ...
    addConditionAddtion() {
      ...
      this.pconditionsAddtion.push(item)
    },
