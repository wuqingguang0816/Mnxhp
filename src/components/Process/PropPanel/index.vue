              ...
              <el-form-item label="驳回步骤">
                <el-select v-model="approverForm.rejectStep" placeholder="请选择" style="width:100%">
                  <el-option v-for="item in rejectStepOptions" :key="item.nodeId"
                    :label="item.title" :value="item.nodeId">
                  </el-option>
                </el-select>
              </el-form-item>
    ...
    */
    ...
    getRejectStepOption() {
      ...
      function transformList(data) {
        if (data.nodeId == _this.value.nodeId) {
          return
        }
        ...
        if (data.childNode) {
          transformList(data.childNode)
        }
      }
      transformList(this.processData)
      this.rejectStepOptions = list
    },
    /**
     * 初始化条件节点数据
     */
